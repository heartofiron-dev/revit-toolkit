// Revit 2024 / .NET Framework 4.8 / C# 7.3
...
namespace PipeHangerTools
{
    static class H
    {
        ...
        {
            var catOk = e.Category != null && e.Category.Id.IntegerValue == (int)BuiltInCategory.OST_MechanicalEquipment;
            if (!catOk) return 0;
    ...
    public class HideHangersOfHiddenPipes : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData c, ref string m, ElementSet s)
        {
            ...
            // 2) 新增功能：对“当前视图里可见”的管，同步取消隐藏其吊架
            //    为了能找回“按元素隐藏”的吊架，这里用全文档收集器，但仍然只围绕每根可见管的近邻去搜，避免全模遍历把电脑卡死了。
            var visiblePipeIds = new FilteredElementCollector(doc, v.Id)
                .OfCategory(BuiltInCategory.OST_PipeCurves)
                .WhereElementIsNotElementType()
                .ToElementIds();

            var toUnhide = new HashSet<ElementId>();

            foreach (var pid in visiblePipeIds)
            {
                var pipe = doc.GetElement(pid) as MEPCurve;
                if (pipe == null) continue;
                ...
                // 仅对这根管的附近建一个outline
                XYZ pMin, pMax; SmallPipeBox(pipe, v, H.NEAR_MM, out pMin, out pMax);
                double pad = UnitUtils.ConvertToInternalUnits(200, UnitTypeId.Millimeters);
                var outline = new Outline(pMin - new XYZ(pad, pad, pad), pMax + new XYZ(pad, pad, pad));
                ...
                {
                    // 属于这根可见管且现在确实被按元素隐藏就取消隐藏
                    if (BelongsToPipeByRules(doc, v, h, pipe) && h.IsHidden(v))
                        toUnhide.Add(h.Id);
                }
            }

            // 避免同一元素既隐藏又取消隐藏
            toUnhide.ExceptWith(toHide);
            ...
            if (toHide.Count == 0 && toUnhide.Count == 0)
            ...
            using (var t = new Transaction(doc, "Hide/Unhide hangers with pipes"))
            {
                ...
                if (toUnhide.Count > 0) v.UnhideElements(toUnhide);
                t.Commit();
            }
            return Result.Succeeded;
        }
